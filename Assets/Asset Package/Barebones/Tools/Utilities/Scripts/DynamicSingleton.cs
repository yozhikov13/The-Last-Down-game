...
namespace Aevien.Utilities
{
    public class DynamicSingleton<T> : MonoBehaviour where T : MonoBehaviour
    {
        ...
        {
            if (Msf.Runtime.SupportsThreads && _initialized && _mainThreadId != -1 && _mainThreadId == Msf.Concurrency.CurrentThreadId)
                return;
            ...
            if (!_initialized)
            {
                ...
                {
                    _mainThreadId = Msf.Concurrency.CurrentThreadId;
                }
            }
        }
