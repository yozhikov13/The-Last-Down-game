...
namespace Barebones.MasterServer
{
    public class MsfChatClient : MsfBaseClient
    {
        ...
        {
            SetHandler((short)MsfMessageCodes.UserJoinedChannel, OnUserJoinedChannelHandler);
            SetHandler((short)MsfMessageCodes.UserLeftChannel, OnUserLeftChannelHandler);
            SetHandler((short)MsfMessageCodes.ChatMessage, OnChatMessageHandler);
        }
        ...
        public void PickUsername(string username, SuccessCallback callback, IClientSocket connection)
        {
            ...
            connection.SendMessage((short)MsfMessageCodes.PickUsername, username, (status, response) =>
            {
        ...
        public void JoinChannel(string channel, SuccessCallback callback, IClientSocket connection)
        {
            ...
            connection.SendMessage((short)MsfMessageCodes.JoinChannel, channel, (status, response) =>
           {
               if (status != ResponseStatus.Success)
               {
                   callback.Invoke(false, response.AsString("Unknown error"));
        ...
        public void LeaveChannel(string channel, SuccessCallback callback, IClientSocket connection)
        {
            ...
            connection.SendMessage((short)MsfMessageCodes.LeaveChannel, channel, (status, response) =>
            {
                if (status != ResponseStatus.Success)
                {
                    callback.Invoke(false, response.AsString("Unknown error"));
        ...
        public void SetDefaultChannel(string channel, SuccessCallback callback, IClientSocket connection)
        {
            ...
            connection.SendMessage((short)MsfMessageCodes.SetDefaultChannel, channel, (status, response) =>
            {
                if (status != ResponseStatus.Success)
                {
                    callback.Invoke(false, response.AsString("Unknown error"));
        ...
        public void GetMyChannels(ChatChannelsCallback callback, IClientSocket connection)
        {
            ...
            connection.SendMessage((short)MsfMessageCodes.GetCurrentChannels, (status, response) =>
            {
                if (status != ResponseStatus.Success)
                {
        ...
        public void GetUsersInChannel(string channel, ChatUsersCallback callback, IClientSocket connection)
        {
            ...
            connection.SendMessage((short)MsfMessageCodes.GetUsersInChannel, channel, (status, response) =>
            {
                if (status != ResponseStatus.Success)
                {
                    callback.Invoke(new List<string>(), response.AsString("Unknown error"));
        ...
        {
            connection.SendMessage((short)MsfMessageCodes.ChatMessage, packet, (status, response) =>
            {
                if (status != ResponseStatus.Success)
                {
                    callback.Invoke(false, response.AsString("Unknown error"));
